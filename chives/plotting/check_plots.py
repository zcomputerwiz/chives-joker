...
from chives.wallet.derive_keys import master_sk_to_joker_farmer_sk, master_sk_to_joker_local_sk, master_sk_to_farmer_sk, master_sk_to_local_sk
...
def check_plots(root_path, num, challenge_start, grep_string, list_duplicates, debug_show_memo):
    ...
    plot_manager.set_public_keys(
        [master_sk_to_farmer_sk(sk).get_g1() for sk, _ in kc.get_all_private_keys()] + [master_sk_to_joker_farmer_sk(sk).get_g1() for sk, _ in kc.get_all_private_keys()],[master_sk_to_farmer_sk(sk).get_g1() for sk, _ in kc.get_all_private_keys()],
